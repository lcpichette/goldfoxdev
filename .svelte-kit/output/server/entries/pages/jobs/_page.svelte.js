import { c as create_ssr_component } from "../../../chunks/index2.js";
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  return `<div class="min-h-full pt-16 pb-12 flex flex-col bg-white"><main class="flex-grow flex flex-col justify-center max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8"><div class="flex-shrink-0 flex justify-center"><a href="/" class="inline-flex"><span class="sr-only">Gold Fox Dev</span>
                <img class="h-12 w-auto" src="/img/Gold-Fox-Dev-Icon-Transparent.webp" alt="Company Icon"></a></div>
        <div class="py-16"><div class="text-center"><p class="text-sm font-semibold text-primary uppercase tracking-wide">No Jobs Available</p>
                  <h1 class="mt-2 text-4xl font-extrabold text-secondary tracking-tight sm:text-5xl">Thank you for your interest!</h1>
                <p class="mt-2 text-base text-secondary-700">There aren&#39;t any job openings for Gold Fox Dev at this moment.</p>
                <div class="mt-6"><a href="/" class="underline-fancy font-semibold text-primary-800">Go back home<span aria-hidden="true">→</span></a></div></div></div></main>
</div>`;
});
export {
  Page as default
};
